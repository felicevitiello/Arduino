...
//definisco gli Input
#define P1    2  //Luce Ingresso e scale
#define P4    4 //Sostituire con 1 oppure con un input mega alto //Luce Bagno Alta  NERO SCIOLTO
#define P6    6 // luce Bagno specchio                                              
#define P3    7 //Luce esterna porta Alta
#define P2    8 //Faretti
#define P5    5 //Campanello
#define in_DHT1 3
...
#define in_POWER 23 //Ibngresso verifica tensione
...
unsigned long webUnixTime (Client &client)
{
  ...
  if (client2.connect("www.gazzetta.it", 80))
    {
    ...
    && client2.readBytes(buf, 5) == 5) // discard
  {
    unsigned day = client2.parseInt();    // day
...
{   
    ...
    while (tempoUnix == 0 && i>0)  {
       tempoUnix =  webUnixTime(client2);
       i--;
    }*/
    
    setTime(tempoUnix);
    Serial.println(tempoUnix);
    ...
    EXECUTEFAST() {                     
      ...
      EXECUTESLOW() {
          UPDATESLOW();
            ...
            if ((subscription = mqtt.readSubscription(400))) {
              if (subscription == &onoffbutton) {
                ...
                message.trim();
                //if (message == "ON") {mInput(RELAY2) = Souliss_T1n_OnCmd;}
                //if (message == "OFF") {mInput(RELAY2) = Souliss_T1n_OffCmd;}
              }
             }
              } 
        SLOW_1day() {
