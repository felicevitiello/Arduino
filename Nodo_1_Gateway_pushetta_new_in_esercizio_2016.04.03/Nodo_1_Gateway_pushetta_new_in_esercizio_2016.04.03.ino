...
#define P4    1 //Sostituire con 1 oppure con un input mega alto //Luce Bagno Alta
...
{ 
    EXECUTEFAST() {                     
          ...
          SHIFT_110ms(0){   // We process the logic and relevant input and output every 50 milliseconds
              ...
              if (IR_AN> IRONOFF) { 
                mInput(IR1) = Souliss_T1n_OnCmd;
                Serial.println("IR1 Attiva");
              }
              else  mInput(IR1) = Souliss_T1n_OffCmd;

              IR_AN = analogRead(in_IR2);
               if (IR_AN> IRONOFF) { 
                mInput(IR2) = Souliss_T1n_OnCmd;
                Serial.println("IR2 Attiva");
              }
              else  mInput(IR2) = Souliss_T1n_OffCmd;

              IR_AN = analogRead(in_IR3);
               if (IR_AN> IRONOFF) { 
                mInput(IR3) = Souliss_T1n_OnCmd;
                Serial.println("IR3 Attiva");
              }
              ...
              if (test){
                ...
                if (mOutput(IR1) == 1)  mInput(L4) =Souliss_T1n_OnCmd;
                if (mOutput(IR1) == 0)  mInput(L4) =Souliss_T1n_OffCmd;
